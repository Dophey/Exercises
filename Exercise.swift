//
//  Exercise.swift
//  
//
//  Created by Sophie on 12/10/2022.
//

import Foundation

struct Exercise {
  var name: String
  var muscleGroups: [String]
  var reps: Int
  var sets: Int
  var totalReps: Int

  init(name: String, muscleGroups: [String], reps: Int, sets: Int) {
    self.name = name
    self.muscleGroups = muscleGroups
    self.reps = reps
    self.sets = sets
    self.totalReps = reps * sets
  }
}

var pushUp: Exercise = Exercise(name: "Push up", muscleGroups: ["Triceps", "Chest", "Shoulders"], reps: 10, sets: 3)

var sitUp: Exercise = Exercise(name: "Sit-up", muscleGroups: ["Core", "Hips", "Thighs"], reps: 10, sets: 3)

var squat: Exercise = Exercise(name: "Squat", muscleGroups: ["Hamstrings", "Glutes", "Quadriceps"], reps: 10, sets: 3)

struct Regimen {
  var dayOfWeek: String
  var exercises: [Exercise]

  init(dayOfWeek: String, exercises: [Exercise]) {
    self.dayOfWeek = dayOfWeek
    self.exercises = exercises
  }

  func printExercisePlan() {
    print("Today is \(self.dayOfWeek) and the plan is to:")
    for exercise in self.exercises {
      print("Do \(exercise.sets) sets of \(exercise.reps) \(exercise.name)s")
      print("That's a total of \(exercise.totalReps) \(exercise.name)s")
    }
  }

}

var mondayRegimen: Regimen = Regimen(dayOfWeek: "Monday", exercises: [pushUp, squat, sitUp])

mondayRegimen.printExercisePlan()

var tuesdayRegimen: Regimen = Regimen(dayOfWeek: "Tuesday", exercises: [sitUp])

print(" ")
tuesdayRegimen.printExercisePlan()

